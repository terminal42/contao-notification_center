...
application.register(
    'terminal42--autosuggester',
    class extends Controller {
        static values = {
            tokens: Array,
        };

        cssClassInputMirror = 'autosuggester-input-mirror';

        cssClassInputMirrorCaret = 'autosuggester-input-mirror-caret';

        cssClassBox = 'autosuggester-box';

        cssClassBoxContainer = 'autosuggester-box-container';

        cssClassBoxList = 'autosuggester-box-list';

        cssClassBoxListItem = 'autosuggester-box-list-item';

        cssClassBoxListItemActive = 'autosuggester-box-list-item-active';

        cssClassBoxListItemValue = 'autosuggester-box-list-item-value';

        cssClassBoxListItemContent = 'autosuggester-box-list-item-content';

        boxListItems = [];

        boxListItemsVisibleIndexes = [];
        ...
        connect() {
            this.tokens = this.tokensValue.map((token) => ({
                name: `##${token.name}##`,
                label: token.label,
            }));
            ...
            setTimeout(() => {
                ...
                if (this.element.tagName.toLowerCase() === 'textarea' && !this.tinyMCEInstance) {
        ...
        disconnect() {
            ...
            if (!this.tinyMCEInstance) {
                this.element.removeEventListener('keyup', this.onKeyUpEvent);
                this.element.removeEventListener('keydown', this.onKeyDownEvent);
            }

            // Allow re-initialization after Turbo navigation
            delete this.element.dataset.autosuggesterId;
        }
        ...
        createBoxElements() {
            ...
            const staleBox = document.getElementById(`${this.element.id}-autosuggester`);
            ...
            this.box.id = `${this.element.id}-autosuggester`;
        ...
        createInputMirror() {
            ...
            this.inputMirror.innerText = this.element.value;
            ...
            properties.forEach((property) => {
                this.inputMirror.style[property] = styles[property];
            });

            // Add the elements to DOM
            this.inputMirror.appendChild(this.inputMirrorCaret);
            this.element.insertAdjacentElement('afterend', this.inputMirror);
        }
        ...
        registerEvents() {
            ...
            if (!this.tinyMCEInstance) {
                this.element.addEventListener('keyup', this.onKeyUpEvent);
                this.element.addEventListener('keydown', this.onKeyDownEvent);
            }

            // Add the events to tinyMCE
            if (this.tinyMCEInstance) {
                if (typeof this.tinyMCEInstance.on === 'function') {
                    this.tinyMCEInstance.on('keyUp', this.onKeyUpEvent);

                    // Fix an issue with the "enter" key
                    this.tinyMCEInstance.off('keyDown');
                    this.tinyMCEInstance.on('keyDown', this.onKeyDownEvent);
                } else {
                    this.tinyMCEInstance.onKeyUp.add((_editor, event) => this.onKeyUpEvent(event));

                    // Fix an issue with the "enter" key
                    this.tinyMCEInstance.onKeyDown.listeners = [];
                    this.tinyMCEInstance.onKeyDown.add((_editor, event) => this.onKeyDownEventonKeyDownEvent(event));
                }
            }

            this.boxListItems.forEach((item) => {
                // Highlight the list item
                item.addEventListener('mouseenter', (e) => this.highlightItem(this.boxListItems.indexOf(e.target)));
        ...
        showBox() {
            ...
            if (this.tinyMCEInstance) {
                ...
                index = this.element.selectionStart;
                ...
                position.top = this.element.getBoundingClientRect().height;
            }
            ...
            if (this.tinyMCEInstance) {
                setPosition(this.boxContainer, this.getPosition(document.getElementById(`${this.element.id}_ifr`)));
            } else {
                setPosition(this.boxContainer, this.getPosition(this.element));
            }

            this.currentListItemIndex = null;
            this.box.style.display = 'block';
            setPosition(this.box, position);
            this.box.scrollTo(0, 0);
            this.boxVisible = true;

            // Use { once: true } so this never accumulates duplicate listeners
            document.body.addEventListener('click', this.hideBox, { once: true });
        }
        ...
        hideBox() {
            ...
            document.body.removeEventListener('click', this.hideBox);
        }
        ...
        onKeyUpEvent(event) {
            ...
            if (this.tinyMCEInstance) {
                ...
                if (
                    ...
                    index += value.length - selection.startContainer.textContent.length;
                }
            } else {
                value = this.element.value;
                index = this.element.selectionStart;
            ...
            const match = this.rgxp.exec(value);

            // Open the box if there is an opening tag
            if (match) {
                this.showBox();
                [this.filterText] = match;
         ...
         */
        highlightItem(index) {
            const maxHeight = parseInt(window.getComputedStyle(this.box).maxHeight, 10);
        ...
        selectItem() {
            ...
            if (this.tinyMCEInstance) {
                ...
                this.element.value = value.slice(0, index) + insert + value.slice(index, value.length);
                this.element.setSelectionRange(indexNew, indexNew);
            }
